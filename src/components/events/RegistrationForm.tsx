	...
	if (submitted) {
		return (
			...
			>
				...
				<h3 className='font-headline text-2xl mb-2'>You&apos;re Registered</h3>
				...
				{calendarLink && (
					<a
						...
						onClick={() =>
							log.info("event_registration_calendar_click", { eventSlug })
						}
		...
		>
			...
			<form
				...
				onSubmit={async (e) => {
					...
					log.info("event_registration_submit", { eventSlug });
					...
					try {
						...
						if (res.ok) {
							log.info("event_registration_success", { eventSlug });
							setSubmitted(true);
						} else {
							const result = await res.json().catch(() => null);
							log.warn("event_registration_api_error", {
								eventSlug,
								status: res.status,
							});
							setError(
			...
			<p className='mt-6 text-xs text-center text-outline leading-relaxed font-body'>
				...
				will receive event-related communications.
			</p>
		</motion.div>
	);
}
