...
export default async function BlogPostPage({ params }: Props) {
	...
	if (!post) {
		...
		notFoundLog.warn("blog_post_not_found", { slug });
	...
	const pageLog = new Logger();
	pageLog.info("blog_post_paywall_gate", {
		slug,
		...
		...(Array.isArray(post.categories) && post.categories.length > 0
			? {
					categories: post.categories.filter(
						(c): c is { title: string; slug: string } => typeof c === "object",
					),
				}
			: {}),
	});
					...
					<PaywallGate isAuthenticated={isAuthenticated} isPremium={false}>
						<div className='editorial-rich-text font-body text-lg text-on-background/90 leading-relaxed'>
							<p className='text-on-surface-variant italic'>
								This is a premium article. Sign in or upgrade to read the full
								content.
							</p>
						</div>
					</PaywallGate>
